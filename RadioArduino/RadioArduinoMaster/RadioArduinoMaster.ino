/**
    ...
    RadioArduinoMaster is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    RadioArduinoMaster is distributed in the hope that it will be useful,
...
void checkSerialInput(String serialIn)
{
    ...
    if (firstSlashOccurrence > 0) {
        ...
        {
            //group/newGroupName - change the current group
            case 1:
                ...
                break;
            //frequency/frequencyValue - change the frequency to all the topic subscribers
            case 2:
                    ...
                    if (commandArg != "") {
                        ...
                        if (newFreq >= 87.0 && newFreq <= 107.0) {
                            if (mqtt_send(command + "/" + String(newFreq)))
                                Serial.println("Frequency update sended: " + String(newFreq));
                        }
                    }
                break;
            ...
            case 4:
                ...
                break;
            //frequencyFollowers/ - send MQTT message to all the topic subscribers to require their current frequency
            case 5:
                    if (mqtt_send(command + "/"))
                        Serial.println("Frequency Followers required");
                break;
            //followersId/ - send MQTT message to all the topic subscribers to require their clientId
            case 7:
                    if (mqtt_send(command + "/"))
                        Serial.println("ID Followers required");
                break;
            //frequencyLocal/frequencyValue - change local frequency
            case 9:
                    //get the frequency value
                    commandArg = serialIn.substring(firstSlashOccurrence+1);
                    commandArg.trim();
    ...
    if (firstSlashOccurrence > 0) {
        ...
        switch(commandMap[command])
        {
            ...
            case 5:
                ...
                break;
            //frequencyAck/ - receive the MQTT messages from subscribers and print their current frequency
            case 6:
                commandArg = mqttIn.substring(firstSlashOccurrence+1);
                commandArg.trim();
                if (commandArg != "") {
                    Serial.print("Frequency ACK received: ");
            ...
            case 7:
                ...
                break;
            //followerIdAck/subscriberId - receive the MQTT message from the subscribers with their client IDs
            case 8:
                commandArg = mqttIn.substring(firstSlashOccurrence+1);
                commandArg.trim();
                if (commandArg != "") {
                    Serial.print("Follower ID ACK received: ");
                    Serial.println(commandArg);
                }
                break;
            //default case if the command is not recognised
